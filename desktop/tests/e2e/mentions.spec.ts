...
test("@ trigger prioritizes channel members before runnable personas and other mentionable agents", async ({
  page,
}) => {
  await installMockBridge(page, {
...
test("relay-only agents stay hidden from channel mentions when not allowlisted", async ({
  page,
}) => {
  await installMockBridge(page, {
    relayAgents: [
      {
        ...
        name: "quinn",
        respondTo: "allowlist",
        respondToAllowlist: ["feedface".repeat(8)],
      },
    ],
  });
  await page.goto("/");
